...
while($row_customer = mysql_fetch_assoc($sq_customer)){
    ...
    $contactNo = $encrypt_decrypt->fnDecrypt($row_customer['contact_no'], $secret_key);
    $emailId = $encrypt_decrypt->fnDecrypt($row_customer['email_id'], $secret_key);

    $birth_date =  ($row_customer['birth_date'] == '1970-01-01') ? 'NA': get_date_user($row_customer['birth_date']);
    $masked_email1 =  ($emailId == "") ? 'NA' : $emailId;
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('H'.$row_count, $contactNo)
