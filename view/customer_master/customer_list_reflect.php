...
while($row_customer = mysql_fetch_assoc($sq_customer)){
	...
	$download_url1 = preg_replace('/(\/+)/','/',$row_customer['id_photo']);
	$download_url1 = BASE_URL.str_replace('../', '', $download_url1);
	$download_url2 = preg_replace('/(\/+)/','/',$row_customer['pro_photo']);
	$download_url2 = BASE_URL.str_replace('../', '', $download_url2);
	
	if($row_customer['id_photo']!=""){
		$id_s = '<a href="'.$download_url1.'" style="padding: 15px 24px;" class="btn btn-info" download><i class="fa fa-download"></i>
		</a>';
	} 
	else 
		$pro_s = '';
	
	if($row_customer['pro_photo']!=""){
		$pro_s = '<a href="'.$download_url2.'" style="padding: 15px 24px;" class="btn btn-info" download><i class="fa fa-download"></i>
		</a>';
	}
	else 
		$id_s = '';
