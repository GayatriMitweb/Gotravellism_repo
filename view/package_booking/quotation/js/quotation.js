...
function group_quotation_cost_calculate () {
	...
	var total =
		parseFloat(adult_cost) + parseFloat(children_cost) + parseFloat(infant_cost) + parseFloat(with_bed_cost);
...
function get_enquiry_details (offset = '') {
	...
	$.ajax({
		...
		success  : function (result) {
			...
			$('#total_passangers' + offset).val(
				parseFloat(result.total_adult) + parseFloat(result.total_children) + parseFloat(result.total_infant)
			);
