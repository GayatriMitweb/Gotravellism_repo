...
?>
		...
		while($row_report = mysql_fetch_assoc($sq_estimate)){
			...
			if($row_report['debit'] != ''){
				$query = "select * from vendor_payment_master where vendor_type='$row_report[vendor_type]' and vendor_type_id = '$row_report[vendor_type_id]'";
				if($estimate_type_id!=""){
					$query .= " and estimate_type_id='$estimate_type_id' ";
				}
				$sq_pay1 = mysql_fetch_assoc(mysql_query($query));
