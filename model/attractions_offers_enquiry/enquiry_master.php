...
function enquiry_master_save(){
  ...
  $mob = $encrypt_decrypt->fnEncrypt($mobile_no, $secret_key);
  $email = $encrypt_decrypt->fnEncrypt($email_id, $secret_key);

  $cust_rows = mysql_num_rows(mysql_query("SELECT * FROM `customer_master` WHERE `contact_no` = '$mob' AND `email_id` = '$email' "));
  if($cust_rows == 0){
    $max_cust_id1 = mysql_fetch_assoc(mysql_query("SELECT max(customer_id) as max FROM customer_master"));
    $max_cust_id1 = $max_cust_id1['max']+1;
    $sq_cust = mysql_query("INSERT INTO `customer_master` (customer_id,first_name,contact_no, email_id) VALUES('$max_cust_id1','$name','$mob','$email') ");

    if(!$sq_cust){
      echo "error--Customer Not Saved.";
      exit;
    }
  }
